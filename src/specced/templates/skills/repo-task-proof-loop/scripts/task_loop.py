...
def guidance_bullets(repo_root: Path, current: Path) -> str:
    ...
    return "\n".join(
        f"- {relative_or_absolute(path, repo_root)}" for path in discovered
    )
...
def install_task_files(
    task_dir: Path, context: dict[str, str], *, force: bool = False
) -> list[str]:
    created: list[str] = []

    file_map = {
        task_dir / "spec.md": render_template(
            load_text_template("spec.md.tmpl"), context
        ),
        task_dir / "evidence.md": render_template(
            load_text_template("evidence.md.tmpl"), context
        ),
        task_dir / "evidence.json": render_template(
            load_text_template("evidence.json.tmpl"), context
        ),
        task_dir / "verdict.json": render_template(
            load_text_template("verdict.json.tmpl"), context
        ),
        task_dir / "problems.md": render_template(
            load_text_template("problems.md.tmpl"), context
        ),
        task_dir / "raw" / "build.txt": load_text_template("raw.build.txt.tmpl"),
        task_dir / "raw" / "test-unit.txt": load_text_template(
            "raw.test-unit.txt.tmpl"
        ),
        task_dir / "raw" / "test-integration.txt": load_text_template(
            "raw.test-integration.txt.tmpl"
        ),
...
def update_guides(
    repo_root: Path, guides: str, install_subagents: str
) -> dict[str, str]:
    ...
    if include_agents:
        guide_targets.append(
            (agents_guide, load_text_template("managed-block-agents.md.tmpl"))
        )
    if include_claude:
        guide_targets.append(
            (claude_guide, load_text_template("managed-block-claude.md.tmpl"))
        )
    ...
    if not isinstance(criteria, list):
        ...
        for index, item in enumerate(criteria):
            if not isinstance(item, dict):
                errors.append(
                    f"evidence.json acceptance_criteria[{index}] must be an object."
                )
                continue
            for key in ("id", "text", "status", "proof", "gaps"):
                if key not in item:
                    errors.append(
                        f"evidence.json acceptance_criteria[{index}] missing key: {key}"
                    )
            if item.get("status") not in STATUS_VALUES:
                errors.append(
                    f"evidence.json acceptance_criteria[{index}].status must be PASS, FAIL, or UNKNOWN."
                )
    return errors
...
def validate_verdict(data: Any, task_id: str) -> list[str]:
    ...
    if not isinstance(criteria, list):
        ...
        for index, item in enumerate(criteria):
            ...
            if item.get("status") not in STATUS_VALUES:
                errors.append(
                    f"verdict.json criteria[{index}].status must be PASS, FAIL, or UNKNOWN."
                )
    return errors
...
def cmd_init(args: argparse.Namespace) -> int:
    ...
    try:
        context = template_context(
            task_id, repo_root, current, args.task_file, args.task_text
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    init_parser = subparsers.add_parser(
        "init", help="Initialize repo-local task artifacts and integration files."
    )
    init_parser.add_argument(
        "--task-id", required=True, help="Task identifier, e.g. feature-auth-hardening"
    )
    init_parser.add_argument(
        "--task-file", help="Optional path to a task description file to seed spec.md"
    )
    init_parser.add_argument(
        "--task-text", help="Optional inline task text to seed spec.md"
    )
    init_parser.add_argument(
        "--repo-root",
        help="Optional working directory inside the repo. Defaults to the current directory.",
    )
    init_parser.add_argument(
        "--guides",
    ...
    init_parser.add_argument(
        "--force",
        action="store_true",
        help="Overwrite existing task artifact templates.",
    )
    init_parser.set_defaults(func=cmd_init)

    validate_parser = subparsers.add_parser(
        "validate", help="Validate required task files and JSON structures."
    )
    validate_parser.add_argument(
        "--task-id", required=True, help="Task identifier to validate."
    )
    validate_parser.add_argument(
        "--repo-root",
        help="Optional working directory inside the repo. Defaults to the current directory.",
    ...
    status_parser = subparsers.add_parser(
        "status", help="Summarize current task artifact status."
    )
    status_parser.add_argument(
        "--task-id", required=True, help="Task identifier to summarize."
    )
    status_parser.add_argument(
        "--repo-root",
        help="Optional working directory inside the repo. Defaults to the current directory.",
